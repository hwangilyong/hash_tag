...
export class JobQueue extends EventEmitter {
  ...
  private canLock(job: AiJob): boolean {
    return job.lockedFiles.every(
      (file) => !this.fileLocks.has(file) || this.fileLocks.get(file) === job.id
    );
  }

  private acquireLocks(job: AiJob) {
    for (const file of job.lockedFiles) this.fileLocks.set(file, job.id);
  }

  private releaseLocks(job: AiJob) {
    for (const file of job.lockedFiles) {
      if (this.fileLocks.get(file) === job.id) this.fileLocks.delete(file);
    }
  }
  ...
  private tick() {
    ...
    for (const id of this.pending) {
      ...
      if (candidate && candidate.status === "blocked" && this.canLock(candidate)) {
